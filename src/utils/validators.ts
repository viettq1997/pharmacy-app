...
export const maxByValue = (field: string, label: string, valueCompare: number, message?: string) =>
    ({getFieldValue}: FormInstance) => ({
        validator(_: any, value: any) {
            if (!value || getFieldValue(field) <= valueCompare) {
                return Promise.resolve();
            }
            return Promise.reject(new Error(message || `${label} must be less than or equal ${valueCompare}!`));
        },
    })
export const minByValue = (field: string, label: string, valueCompare: number, message?: string) =>
    ({getFieldValue}: FormInstance) => ({
        validator(_: any, value: any) {
            if (value == null || getFieldValue(field) >= valueCompare) {
                return Promise.resolve();
            }
            return Promise.reject(new Error(message || `${label} must be greater than or equal ${valueCompare}!`));
        },
    })
export const maxByField = (field: string, label: string, fieldCompare: string, message?: string) =>
    ({getFieldValue}: FormInstance) => ({
        validator(_: any, value: any) {
            const valueCompare = getFieldValue(fieldCompare)
            if (value == null || !valueCompare || getFieldValue(field) <= valueCompare) {
                return Promise.resolve();
            }
            return Promise.reject(new Error(message || `${label} must be less than or equal ${valueCompare}!`));
        },
    })
export const minByField = (field: string, label: string, fieldCompare: string, message?: string) =>
    ({getFieldValue}: FormInstance) => ({
        validator(_: any, value: any) {
            const valueCompare = getFieldValue(fieldCompare)
            if (value == null || valueCompare == null || getFieldValue(field) >= valueCompare) {
                return Promise.resolve();
            }
            return Promise.reject(new Error(message || `${label} must be greater than or equal ${valueCompare}!`));
        },
